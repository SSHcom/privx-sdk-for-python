...
def get_host_data():
    ...
    if resp.ok():
        ...
        for host_data in data_items:
            ...
            hosts_data["users"] =  ','.join([u['principal'] if u['principal'] else "DirectoryUser" for u in host_data['principals']])
            hosts_data["protocol"] = ','.join(list(dict.fromkeys([p['service'] for p in host_data['services']])))
            for p in data_list:
